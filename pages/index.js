  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, padding:"56px 24px 40px", display:"flex", flexDirection:"column", overflowY:"auto" }}>
      ...
      <div style={{ marginBottom:20 }}>
        ...
        <div style={{ background:C.card, border:`1.5px solid ${C.cardBorder}`, borderRadius:14, padding:"4px 16px", display:"flex", alignItems:"center", gap:10 }}>
          <input value={name} onChange={e=>setName(e.target.value)} placeholder="z.B. Mama, Papa, Ich..." style={{ flex:1, fontSize:15, padding:"13px 0" }}/>
        </div>
      </div>
      <div style={{ marginBottom:20 }}>
        ...
        <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
          {EMOJIS.map(e=>(
            <button key={e} onClick={()=>setEmoji(e)} style={{ width:44, height:44, borderRadius:12, fontSize:22, border:`2px solid ${emoji===e?C.accent:C.cardBorder}`, background:emoji===e?C.accentGlow:C.card, transition:"all 0.2s" }}>{e}</button>
          ))}
        </div>
      </div>
      <div style={{ marginBottom:20 }}>
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:8, marginBottom:12 }}>
          {DIET_OPTIONS.map(({l,e})=>(
            <TagToggle key={l} label={l} emoji={e} selected={diet.includes(l)} color={C.danger} glowColor={C.dangerGlow} onClick={()=>toggle(l)}/>
          ))}
        </div>
        ...
        {custom.length>0 && (
          <div style={{ display:"flex", flexWrap:"wrap", gap:6, marginTop:10 }}>
            {custom.map(c=><Chip key={c} label={c} color={C.danger} dimColor="#b03030" glowColor={C.dangerGlow} onRemove={()=>setCustom(p=>p.filter(x=>x!==c))}/>)}
          </div>
        )}
      </div>
      <div style={{flex:1}}/>
...
function ProfileManager({ profiles, onSave, onBack }) {
  ...
  if (editing) {
    return (
      <ProfileEditor profile={editing==="new"?null:editing} isNew={editing==="new"}
        onSave={(p)=>{ const updated=editing==="new"?[...profiles,p]:profiles.map(x=>x.id===p.id?p:x); onSave(updated); setEditing(null); }}
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, padding:"56px 24px 40px", display:"flex", flexDirection:"column" }}>
      ...
      <p style={{ color:C.textMuted, fontSize:14, marginBottom:28 }}>Jedes Profil hat eigene Ernährungsregeln.</p>
        ...
        {profiles.map(p=>(
          <div key={p.id} style={{ background:C.card, border:`1px solid ${C.cardBorder}`, borderRadius:16, padding:"16px 18px", display:"flex", alignItems:"center", gap:14 }}>
            ...
            <div style={{flex:1}}>
              ...
              {([...p.diet||[],...p.custom||[]]).length>0 && (
                <p style={{fontSize:12,color:C.textMuted,marginTop:3}}>{[...p.diet,...p.custom].join(" · ")}</p>
              )}
            </div>
      ...
      <button onClick={()=>setEditing("new")} style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:10, padding:"16px", borderRadius:16, border:`1.5px dashed ${C.cardBorder}`, color:C.textMuted, fontSize:15, fontFamily:BODY, background:"transparent", width:"100%" }}>
        ➕ Neues Profil
      </button>
    </div>
  );
      ...
      {profiles.length>0 ? (
        <div style={{ width:"100%", maxWidth:320, marginBottom:24 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {profiles.map(p=>(
              <button key={p.id} onClick={()=>setSelected(p.id)} style={{ display:"flex", alignItems:"center", gap:14, padding:"14px 18px", borderRadius:16, border:`2px solid ${selected===p.id?C.accent:C.cardBorder}`, background:selected===p.id?C.accentGlow:C.card, transition:"all 0.2s", width:"100%", textAlign:"left" }}>
                <span style={{fontSize:28}}>{p.emoji}</span>
                <div style={{flex:1}}>
                  <p style={{ fontWeight:600, fontSize:15, color:selected===p.id?C.accent:C.text }}>{p.name}</p>
                  {([...p.diet||[],...p.custom||[]]).length>0 && (
                    <p style={{ fontSize:12, color:C.textMuted, marginTop:2 }}>{[...p.diet,...p.custom].slice(0,3).join(" · ")}{[...p.diet,...p.custom].length>3?" …":""}</p>
                  )}
                </div>
        ...
        <div style={{ marginBottom:24, padding:20, background:C.card, border:`1px dashed ${C.cardBorder}`, borderRadius:18, maxWidth:300 }}>
          <p style={{ color:C.textMuted, fontSize:14, lineHeight:1.6 }}>Erstelle ein Profil um Unverträglichkeiten und Ernährungspräferenzen dauerhaft zu speichern.</p>
        </div>
      )}
      <div style={{ display:"flex", flexDirection:"column", gap:10, width:"100%", maxWidth:320 }}>
        <button onClick={()=>onStart(profiles.find(p=>p.id===selected)||null)} style={{ background:`linear-gradient(135deg,${C.accent},${C.accentDim})`, color:"#0f0e0c", fontWeight:700, fontSize:16, padding:"16px 52px", borderRadius:50, fontFamily:BODY, boxShadow:`0 8px 32px rgba(245,166,35,0.3)` }}>
          Los geht's →
        </button>
...
function IngredientsScreen({ onNext, onSkip }) {
  ...
  const handleScan = async (e) => {
    ...
    try {
      const compressed = await new Promise((res,rej)=>{
        ...
        img.onload=()=>{ const canvas=document.createElement("canvas"); const MAX=1024; let w=img.width,h=img.height; if(w>MAX||h>MAX){if(w>h){h=Math.round(h*MAX/w);w=MAX;}else{w=Math.round(w*MAX/h);h=MAX;}} canvas.width=w;canvas.height=h;canvas.getContext("2d").drawImage(img,0,0,w,h);URL.revokeObjectURL(url);res(canvas.toDataURL("image/jpeg",0.8).split(",")[1]); };
      ...
      if(data.ingredients?.length){ setIngredients(prev=>{const c=[...prev];data.ingredients.forEach(f=>{if(!c.includes(f))c.push(f);});return c;}); setScanDone(true); }
      else setScanError(true);
    } catch(err){ console.error(err); setScanError(true); }
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, padding:"56px 24px 32px", display:"flex", flexDirection:"column" }}>
      ...
      <label style={{ background:scanning?C.surface:`linear-gradient(135deg,rgba(245,166,35,0.18),rgba(196,125,14,0.1))`, border:`1.5px dashed ${scanning?C.cardBorder:scanError?C.danger:C.accentDim}`, borderRadius:18, padding:"18px 16px", marginBottom:14, display:"flex", alignItems:"center", gap:14, width:"100%", cursor:scanning?"default":"pointer", boxSizing:"border-box", transition:"all 0.3s" }}>
        <input type="file" accept="image/*" onChange={handleScan} disabled={scanning} style={{display:"none"}}/>
        {scanning?(<><div style={{width:36,height:36,borderRadius:"50%",border:`3px solid ${C.accentDim}`,borderTopColor:C.accent,animation:"spin 0.8s linear infinite",flexShrink:0}}/><div><p style={{color:C.accent,fontWeight:600,fontSize:14,marginBottom:2}}>KI analysiert Foto...</p><p style={{color:C.textMuted,fontSize:12}}>Zutaten werden erkannt</p></div></>)
        :scanDone?(<><div style={{width:36,height:36,borderRadius:12,background:C.greenGlow,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>✅</div><div><p style={{color:C.green,fontWeight:600,fontSize:14,marginBottom:2}}>Zutaten erkannt!</p><p style={{color:C.textMuted,fontSize:12}}>Erneut scannen für mehr</p></div></>)
        :scanError?(<><div style={{width:36,height:36,borderRadius:12,background:C.dangerGlow,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>❌</div><div><p style={{color:C.danger,fontWeight:600,fontSize:14,marginBottom:2}}>Scan fehlgeschlagen</p><p style={{color:C.textMuted,fontSize:12}}>Erneut versuchen oder manuell eingeben</p></div></>)
      ...
      <div style={{marginBottom:24}}>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
          {suggestions.filter(s=>!ingredients.includes(s)).map(s=>(<button key={s} onClick={()=>setIngredients(p=>[...p,s])} style={{background:C.surface,border:`1px solid ${C.cardBorder}`,borderRadius:20,padding:"7px 14px",color:C.textMuted,fontSize:13,fontFamily:BODY}}>{s}</button>))}
        </div>
      </div>
      <div style={{flex:1}}/>
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
...
function DislikedScreen({ onNext, onBack }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, padding:"56px 24px 32px", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ marginBottom:24 }}>
        <h2 style={{ fontFamily:DISPLAY, fontSize:28, fontWeight:700, lineHeight:1.2, marginBottom:6 }}>Heute keine Lust auf...?</h2>
        <p style={{ color:C.textMuted, fontSize:14 }}>Optional – nur für heute.</p>
      </div>
      <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:14}}>
        {DISLIKE.map(({l,e})=><TagToggle key={l} label={l} emoji={e} selected={disliked.includes(l)} color={C.textMuted} glowColor={C.surface} onClick={()=>toggle(l)}/>)}
      </div>
      ...
      <div style={{flex:1}}/>
      <BigButton label="Weiter →" onClick={()=>onNext(disliked)}/>
    </div>
  );
}
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg}}>
      <div style={{background:`linear-gradient(180deg,rgba(245,166,35,0.1) 0%,transparent 100%)`,padding:"60px 24px 28px",borderBottom:`1px solid ${C.cardBorder}`,animation:"fadeUp 0.5s ease"}}>
        ...
        {(allRestrictions.length>0||disliked?.length>0)&&(
          <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
            {allRestrictions.map(r=><Chip key={r} label={r} color={C.danger} dimColor="#b03030" glowColor={C.dangerGlow}/>)}
            {disliked?.map(d=><Chip key={d} label={`Kein ${d}`} color={C.textMuted} dimColor={C.textDim} glowColor={C.surface}/>)}
          </div>
        )}
      </div>
      <div style={{padding:"22px 24px 140px",display:"flex",flexDirection:"column",gap:18}}>
        <div style={{background:C.card,border:`1px solid ${C.cardBorder}`,borderRadius:18,padding:18,animation:"fadeUp 0.5s 0.1s ease both"}}>
          ...
          {missing.length>0&&(<>
            <button onClick={()=>setShowShopping(!showShopping)} style={{marginTop:12,width:"100%",padding:"10px",borderRadius:10,background:C.accentGlow,border:`1px solid ${C.accentDim}`,color:C.accent,fontSize:13,fontWeight:600,fontFamily:BODY}}>🛒 Einkaufsliste ({missing.length}) {showShopping?"▲":"▼"}</button>
...
export default function Mahlzeit() {
  ...
  const saveAndUpdate=(updated)=>{ setProfiles(updated); saveProfiles(updated); };

  const callAPI=async(finalPrefs,nope=null)=>{
    setScreen("loading");
    const restrictions=[...(activeProfile?.diet||[]),...(activeProfile?.custom||[])];
    try {
      const resp=await fetch("/api/claude",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:"recipe",ingredients,time:finalPrefs.time,mood:finalPrefs.mood,portion:finalPrefs.portion,intolerances:restrictions,disliked,nope})});
      const data=await resp.json();
