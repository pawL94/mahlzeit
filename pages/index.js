...
const store = {
  ...
  recipes: {
    ...
    save: (profileId, r) => { try { localStorage.setItem("mz_recipes_" + (profileId || "global"), JSON.stringify(r)); } catch(e) {} },
    load: (profileId) => { try { return JSON.parse(localStorage.getItem("mz_recipes_" + (profileId || "global")) || "[]"); } catch(e) { return []; } },
  },
  week: {
    save: (profileId, w) => { try { localStorage.setItem("mz_week_" + (profileId || "global"), JSON.stringify(w)); } catch(e) {} },
    load: (profileId) => { try { return JSON.parse(localStorage.getItem("mz_week_" + (profileId || "global")) || "null"); } catch(e) { return null; } },
  },
};
...
const DIET_OPTIONS = [
  ...
  {l:"Eierallergie",e:"🥚"},{l:"Sojaallergie",e:"🫘"},{l:"Keine Meeresfruechte",e:"🦐"},
];

function ProfileEditor({ profile, onSave, onCancel, isNew }) {
  const [name, setName] = useState(profile ? profile.name : "");
  const [emoji, setEmoji] = useState(profile ? profile.emoji : "🧑");
  const [diet, setDiet] = useState(profile ? profile.diet : []);
  const [custom, setCustom] = useState(profile ? profile.custom : []);
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, padding:"56px 24px 40px", display:"flex", flexDirection:"column", overflowY:"auto" }}>
      <button onClick={onCancel} style={{ color:C.textMuted, fontSize:14, marginBottom:20, display:"flex", alignItems:"center", gap:6 }}>{"<"} Zurueck</button>
      <h2 style={{ fontFamily:DISPLAY, fontSize:26, fontWeight:700, marginBottom:6 }}>{isNew ? "Neues Profil" : "Profil bearbeiten"}</h2>
      <p style={{ color:C.textMuted, fontSize:14, marginBottom:24 }}>Einmal einrichten, immer beruecksichtigt.</p>
      ...
      <div style={{ marginBottom:20 }}>
        <SectionLabel>Ernaehrung und Unvertraeglichkeiten</SectionLabel>
        ...
        <div style={{ background:C.card, border:"1px solid " + C.cardBorder, borderRadius:14, padding:"4px 14px", display:"flex", alignItems:"center", gap:8 }}>
          <input value={customInput} onChange={e=>setCustomInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();addCustom();}}} placeholder="Weitere Einschraenkung..." style={{ flex:1, fontSize:14, padding:"12px 0" }}/>
      ...
      <div style={{flex:1}}/>
      <BigButton label={isNew ? "Profil erstellen" : "Speichern"} onClick={()=>{ if(name.trim()) onSave({ name:name.trim(), emoji, diet, custom, id:profile ? profile.id : Date.now() }); }} disabled={!name.trim()}/>
    </div>
  );
}
...
function ProfileManager({ profiles, onSave, onBack }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, padding:"56px 24px 40px", display:"flex", flexDirection:"column" }}>
      <button onClick={onBack} style={{ color:C.textMuted, fontSize:14, marginBottom:20, display:"flex", alignItems:"center", gap:6 }}>{"<"} Zurueck</button>
      <h2 style={{ fontFamily:DISPLAY, fontSize:26, fontWeight:700, marginBottom:6 }}>Profile</h2>
      <p style={{ color:C.textMuted, fontSize:14, marginBottom:28 }}>Jedes Profil hat eigene Rezepte und Einstellungen.</p>
        ...
        {profiles.map(p=>(
          <div key={p.id} style={{ background:C.card, border:"1px solid " + C.cardBorder, borderRadius:16, padding:"16px 18px", display:"flex", alignItems:"center", gap:14 }}>
            ...
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>setEditing(p)} style={{color:C.accent,fontSize:13,fontFamily:BODY,padding:"6px 12px",background:C.accentGlow,borderRadius:8}}>Bearbeiten</button>
              <button onClick={()=>onSave(profiles.filter(x=>x.id!==p.id))} style={{color:C.danger,fontSize:13,fontFamily:BODY,padding:"6px 12px",background:C.dangerGlow,borderRadius:8}}>X</button>
            </div>
          </div>
        ))}
      </div>
...
function SplashScreen({ profiles, onStart, onManageProfiles, onViewSaved, onWeekPlanner }) {
  ...
  return (
    <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:"radial-gradient(ellipse at 60% 20%, rgba(245,166,35,0.08) 0%, transparent 60%), " + C.bg, padding:32, textAlign:"center" }}>
      ...
      <p style={{ color:C.textMuted, fontSize:14, marginBottom:32 }}>Nie wieder gruebeln was du kochen sollst.</p>
      {profiles.length>0 ? (
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {profiles.map(p=>(
              <button key={p.id} onClick={()=>setSelected(p.id)} style={{ display:"flex", alignItems:"center", gap:14, padding:"14px 18px", borderRadius:16, border:"2px solid " + (selected===p.id ? C.accent : C.cardBorder), background:selected===p.id ? C.accentGlow : C.card, width:"100%", textAlign:"left" }}>
                ...
                {selected===p.id && <span style={{color:C.accent,fontSize:18}}>OK</span>}
              </button>
            ))}
          </div>
        </div>
        ...
        <div style={{ marginBottom:24, padding:20, background:C.card, border:"1px dashed " + C.cardBorder, borderRadius:18, maxWidth:300 }}>
          <p style={{ color:C.textMuted, fontSize:14, lineHeight:1.6 }}>Erstelle ein Profil um Unvertraeglichkeiten und Rezepte dauerhaft zu speichern.</p>
        </div>
      )}
      <div style={{ display:"flex", flexDirection:"column", gap:10, width:"100%", maxWidth:320 }}>
        <button onClick={()=>onStart(profiles.find(p=>p.id===selected)||null)} style={{ background:"linear-gradient(135deg," + C.accent + "," + C.accentDim + ")", color:"#0f0e0c", fontWeight:700, fontSize:16, padding:"16px 52px", borderRadius:50, fontFamily:BODY, boxShadow:"0 8px 32px rgba(245,166,35,0.3)" }}>Los geht es</button>
        ...
        <button onClick={onManageProfiles} style={{ color:C.textMuted, fontSize:14, padding:"10px", fontFamily:BODY }}>{profiles.length>0 ? "Profile verwalten" : "Profil erstellen"}</button>
...
function IngredientsScreen({ onNext, onSkip }) {
  ...
  const handleScan = async (e) => {
    ...
    try {
      ...
      if(data.ingredients&&data.ingredients.length){ setIngredients(prev=>{const c=[...prev];data.ingredients.forEach(f=>{if(!c.includes(f))c.push(f);});return c;}); setScanDone(true); } else setScanError(true);
    } catch(err){ setScanError(true); } finally{ setScanning(false); e.target.value=""; }
  };
  const suggestions=["Eier","Nudeln","Tomaten","Kaese","Haehnchen","Zwiebeln","Knoblauch","Reis","Kartoffeln","Paprika","Speck","Lachs","Tofu","Linsen"];
  return (
    <div style={{ minHeight:"100vh", background:C.bg, padding:"56px 24px 32px", display:"flex", flexDirection:"column" }}>
      <ProgressBar step={1} total={3}/>
      <div style={{ marginBottom:20 }}><h2 style={{ fontFamily:DISPLAY, fontSize:28, fontWeight:700, lineHeight:1.2, marginBottom:6 }}>Was hast du im Kuehlschrank?</h2><p style={{ color:C.textMuted, fontSize:14 }}>Foto scannen, eintippen oder ueberspringen.</p></div>
      <label style={{ background:scanning ? C.surface : "linear-gradient(135deg,rgba(245,166,35,0.18),rgba(196,125,14,0.1))", border:"1.5px dashed " + (scanning ? C.cardBorder : scanError ? C.danger : C.accentDim), borderRadius:18, padding:"18px 16px", marginBottom:14, display:"flex", alignItems:"center", gap:14, width:"100%", cursor:scanning ? "default" : "pointer", boxSizing:"border-box" }}>
        ...
        : scanDone ? (<><div style={{width:36,height:36,borderRadius:12,background:C.greenGlow,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>OK</div><div><p style={{color:C.green,fontWeight:600,fontSize:14,marginBottom:2}}>Zutaten erkannt!</p><p style={{color:C.textMuted,fontSize:12}}>Erneut scannen fuer mehr</p></div></>)
        ...
        : (<><div style={{width:36,height:36,borderRadius:12,background:C.accentGlow,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>📸</div><div><p style={{color:C.accent,fontWeight:600,fontSize:14,marginBottom:2}}>Kuehlschrank scannen</p><p style={{color:C.textMuted,fontSize:12}}>KI erkennt Zutaten automatisch</p></div></>)}
      </label>
...
function DislikedScreen({ onNext, onBack }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, padding:"56px 24px 32px", display:"flex", flexDirection:"column" }}>
      <button onClick={onBack} style={{ color:C.textMuted, fontSize:14, marginBottom:14, display:"flex", alignItems:"center", gap:6 }}>{"<"} Zurueck</button>
      <ProgressBar step={2} total={3}/>
      <div style={{ marginBottom:24 }}><h2 style={{ fontFamily:DISPLAY, fontSize:28, fontWeight:700, lineHeight:1.2, marginBottom:6 }}>Heute keine Lust auf...?</h2><p style={{ color:C.textMuted, fontSize:14 }}>Optional - nur fuer heute.</p></div>
...
function LoadingScreen() {
  ...
  const steps=["Analysiere deine Praeferenzen...","Kreiere das perfekte Rezept...","Fast fertig..."];
  ...
  return (
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:32,textAlign:"center",background:"radial-gradient(ellipse at 50% 40%, rgba(245,166,35,0.06) 0%, transparent 60%), " + C.bg}}>
      ...
      <h2 style={{fontFamily:DISPLAY,fontSize:26,fontWeight:700,marginBottom:12}}>Ich koche fuer dich...</h2>
...
function RecipeScreen({ recipe, profile, disliked, onNope, onRestart, onBack, onViewSaved }) {
  ...
  const missing=recipe.ingredients ? recipe.ingredients.filter(i=>!i.available) : [];
  const allRestrictions=[...((profile&&profile.diet)||[]),...((profile&&profile.custom)||[])];

  const handleSave = () => {
    const recipes=store.recipes.load(profile ? profile.id : null);
    const entry=Object.assign({},recipe,{id:Date.now(),savedAt:new Date().toLocaleDateString("de-DE"),profileId:profile ? profile.id : null,status:"saved"});
    store.recipes.save(profile ? profile.id : null,[entry,...recipes.slice(0,49)]);
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg}}>
      <div style={{background:"linear-gradient(180deg,rgba(245,166,35,0.1) 0%,transparent 100%)",padding:"60px 24px 28px",borderBottom:"1px solid " + C.cardBorder,animation:"fadeUp 0.5s ease"}}>
        <button onClick={onBack} style={{color:C.textMuted,fontSize:14,display:"flex",alignItems:"center",gap:6,marginBottom:14}}>{"<"} Einstellungen aendern</button>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:(allRestrictions.length||(disliked&&disliked.length)) ? 12 : 0}}>
          {[["Zeit",recipe.time],["Level",recipe.difficulty],["Kalorien",recipe.calories]].map(([icon,val])=>(<div key={val} style={{background:C.card,border:"1px solid " + C.cardBorder,borderRadius:10,padding:"7px 12px",display:"flex",alignItems:"center",gap:5,fontSize:12,color:C.textMuted}}>{icon}: {val}</div>))}
        </div>
        {(allRestrictions.length>0||(disliked&&disliked.length>0)) && <div style={{display:"flex",flexWrap:"wrap",gap:6}}>{allRestrictions.map(r=><Chip key={r} label={r} color={C.danger} dimColor="#b03030" glowColor={C.dangerGlow}/>)}{(disliked||[]).map(d=><Chip key={d} label={"Kein " + d} color={C.textMuted} dimColor={C.textDim} glowColor={C.surface}/>)}</div>}
      </div>
      ...
      <div style={{padding:"22px 24px 140px",display:"flex",flexDirection:"column",gap:18}}>
        <div style={{background:C.card,border:"1px solid " + C.cardBorder,borderRadius:18,padding:18}}>
          ...
          {recipe.ingredients && recipe.ingredients.map((ing,i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"9px 0",borderBottom:i<recipe.ingredients.length-1 ? "1px solid " + C.cardBorder : "none"}}><span style={{fontSize:14,color:ing.available ? C.text : C.accent}}>{ing.available ? "OK" : "Kaufen"} {ing.name}</span><span style={{fontSize:13,color:C.textMuted}}>{ing.amount}</span></div>))}
          {missing.length>0 && (<><button onClick={()=>setShowShopping(!showShopping)} style={{marginTop:12,width:"100%",padding:"10px",borderRadius:10,background:C.accentGlow,border:"1px solid " + C.accentDim,color:C.accent,fontSize:13,fontWeight:600,fontFamily:BODY}}>Einkaufsliste ({missing.length}) {showShopping ? "weniger" : "anzeigen"}</button>{showShopping && <div style={{marginTop:10,padding:12,background:C.surface,borderRadius:10}}>{missing.map((ing,i)=><div key={i} style={{fontSize:13,color:C.accent,padding:"4px 0"}}>- {ing.name}: {ing.amount}</div>)}</div>}</>)}
        </div>
        <div style={{background:C.card,border:"1px solid " + C.cardBorder,borderRadius:18,padding:18}}>
          <h3 style={{fontFamily:DISPLAY,fontSize:17,marginBottom:14}}>Zubereitung</h3>
          <div style={{display:"flex",flexDirection:"column",gap:16}}>{recipe.steps && recipe.steps.map((step,i)=>(<div key={i} style={{display:"flex",gap:14,alignItems:"flex-start"}}><div style={{minWidth:28,height:28,borderRadius:"50%",background:"linear-gradient(135deg," + C.accent + "," + C.accentDim + ")",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,color:"#0f0e0c",flexShrink:0}}>{i+1}</div><p style={{fontSize:14,color:C.textMuted,lineHeight:1.6,paddingTop:4}}>{step}</p></div>))}</div>
        </div>
        {recipe.tip && <div style={{background:C.accentGlow,border:"1px solid " + C.accentDim,borderRadius:14,padding:14}}><p style={{fontSize:13,color:C.accent,lineHeight:1.6}}>Tipp: {recipe.tip}</p></div>}
      </div>
      ...
      {showNope && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",display:"flex",alignItems:"flex-end",zIndex:100}} onClick={()=>setShowNope(false)}>
          <div onClick={e=>e.stopPropagation()} style={{background:C.card,borderRadius:"24px 24px 0 0",padding:"28px 24px 52px",width:"100%",border:"1px solid " + C.cardBorder,animation:"slideUp 0.35s ease",maxWidth:430,margin:"0 auto"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {[{label:"Zu aufwendig",desc:"Zeig mir was Schnelleres",emoji:"😴",reason:"zu_aufwendig"},{label:"Anderes Gericht",desc:"Komplett andere Kueche",emoji:"🔄",reason:"anderes_gericht"},{label:"Lieber bestellen",desc:"Oeffnet Lieferando",emoji:"🛵",reason:"bestellen"}].map(({label,desc,emoji,reason})=>(
...
function SavedRecipesScreen({ profile, profiles, onBack, onOpen }) {
  ...
  useEffect(()=>{ setRecipes(store.recipes.load(profile ? profile.id : null)); },[profile]);

  const updateRecipes = (updated) => { setRecipes(updated); store.recipes.save(profile ? profile.id : null, updated); };

  const rate = (id, rating) => {
    const updated = recipes.map(r => {
      if(r.id!==id) return r;
      if(rating==="up") return Object.assign({},r,{status:"loved",ratedAt:new Date().toLocaleDateString("de-DE")});
      return null;
    }).filter(Boolean);
  ...
  const moveToProfile = (recipe, targetProfileId) => {
    ...
    store.recipes.save(targetProfileId,[Object.assign({},recipe,{profileId:targetProfileId}),...targetRecipes]);
    setMoveTarget(null);
  };

  const shown = tab==="loved" ? recipes.filter(r=>r.status==="loved") : recipes.filter(r=>r.status==="saved");
  const otherProfiles = profiles.filter(p=>p.id!==(profile ? profile.id : null));
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,padding:"56px 24px 32px",display:"flex",flexDirection:"column"}}>
      ...
      <p style={{color:C.textMuted,fontSize:13,marginBottom:20}}>Gespeichert auf diesem Geraet.</p>
      <div style={{display:"flex",gap:8,marginBottom:20}}>
        {[["saved","Gespeichert"],["loved","Lieblingsgerichte"]].map(([val,label])=>(
              ...
              <div style={{padding:"16px 18px"}}>
                ...
                {tab==="saved" && (
                  <div style={{background:C.surface,borderRadius:12,padding:"12px 14px",marginBottom:12}}>
                    <p style={{color:C.textMuted,fontSize:12,marginBottom:8}}>Hast du es schon gekocht? Wie wars?</p>
                    <div style={{display:"flex",gap:8}}>
                      <button onClick={()=>rate(r.id,"up")} style={{flex:1,padding:"10px",borderRadius:10,background:C.greenGlow,border:"1px solid " + C.green,color:C.green,fontSize:14,fontFamily:BODY,fontWeight:600}}>Hat geschmeckt!</button>
                      <button onClick={()=>rate(r.id,"down")} style={{flex:1,padding:"10px",borderRadius:10,background:C.dangerGlow,border:"1px solid " + C.danger,color:C.danger,fontSize:14,fontFamily:BODY,fontWeight:600}}>Nicht meins</button>
                    </div>
                  </div>
                )}
                <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                  <button onClick={()=>onOpen(r)} style={{flex:1,padding:"10px",borderRadius:12,background:"linear-gradient(135deg," + C.accent + "," + C.accentDim + ")",color:"#0f0e0c",fontWeight:700,fontSize:13,fontFamily:BODY}}>Rezept oeffnen</button>
                  {otherProfiles.length>0 && <button onClick={()=>setMoveTarget(moveTarget && moveTarget.id===r.id ? null : r)} style={{padding:"10px 12px",borderRadius:12,background:C.purpleGlow,color:C.purple,fontSize:13,fontFamily:BODY,border:"1px solid " + C.purple}}>Verschieben</button>}
                  <button onClick={()=>updateRecipes(recipes.filter(x=>x.id!==r.id))} style={{padding:"10px 12px",borderRadius:12,background:C.dangerGlow,color:C.danger,fontSize:13,fontFamily:BODY}}>X</button>
                </div>
                {moveTarget && moveTarget.id===r.id && otherProfiles.length>0 && (
...
function WeekPlanner({ profile, onBack }) {
  ...
  const [week,setWeek]=useState(()=>store.week.load(profile ? profile.id : null)||emptyWeek());
  const [generatingAll,setGeneratingAll]=useState(false);

  const generateDay = async (dayIndex, currentWeek) => {
    setWeek(prev=>prev.map((d,i)=>i===dayIndex ? Object.assign({},d,{loading:true}) : d));
    const restrictions=[...((profile&&profile.diet)||[]),...((profile&&profile.custom)||[])];
    const lovedRecipes=store.recipes.load(profile ? profile.id : null).filter(r=>r.status==="loved").slice(0,8).map(r=>r.name);
    const usedThisWeek=(currentWeek||week).filter((_,i)=>i!==dayIndex).map(d=>d.recipe ? d.recipe.name : null).filter(Boolean);
    try {
      ...
      const data=await resp.json();
      if(data.recipe){
        setWeek(prev=>{const updated=prev.map((d,i)=>i===dayIndex ? Object.assign({},d,{recipe:data.recipe,loading:false}) : d);store.week.save(profile ? profile.id : null,updated);return updated;});
      } else {
        setWeek(prev=>prev.map((d,i)=>i===dayIndex ? Object.assign({},d,{loading:false}) : d));
      }
    } catch(e){ setWeek(prev=>prev.map((d,i)=>i===dayIndex ? Object.assign({},d,{loading:false}) : d)); }
  };

  const generateAll = async () => {
    setGeneratingAll(true);
    const fresh=DAYS.map(day=>({day,recipe:null,loading:false}));
    setWeek(fresh);
    for(let i=0;i<7;i++){
      await generateDay(i,fresh);
    }
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,padding:"56px 24px 32px",display:"flex",flexDirection:"column"}}>
      ...
      <button onClick={generateAll} disabled={generatingAll} style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,padding:"14px",borderRadius:50,background:generatingAll ? C.surface : "linear-gradient(135deg," + C.accent + "," + C.accentDim + ")",color:generatingAll ? C.textDim : "#0f0e0c",fontWeight:700,fontSize:15,fontFamily:BODY,marginBottom:24,boxShadow:generatingAll ? "none" : "0 8px 24px rgba(245,166,35,0.25)"}}>
        {generatingAll ? "Wird generiert..." : "Ganze Woche generieren"}
      </button>
      <div style={{display:"flex",flexDirection:"column",gap:12,overflowY:"auto"}}>
        {week.map((d,i)=>(
          <div key={d.day} style={{background:C.card,border:"1px solid " + C.cardBorder,borderRadius:18,padding:"16px 18px"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:d.recipe ? 12 : 0}}>
              ...
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                {d.recipe && <button onClick={()=>saveRecipe(d.recipe)} style={{color:C.green,fontSize:12,fontFamily:BODY,padding:"5px 10px",background:C.greenGlow,borderRadius:8}}>Speichern</button>}
                <button onClick={()=>generateDay(i,week)} disabled={d.loading||generatingAll} style={{display:"flex",alignItems:"center",gap:6,padding:"7px 12px",borderRadius:10,background:C.accentGlow,border:"1px solid " + C.accentDim,color:C.accent,fontSize:12,fontFamily:BODY,fontWeight:600}}>
                  {d.loading ? "..." : d.recipe ? "Neu" : "Generieren"}
                </button>
              </div>
            </div>
            {d.loading ? (
              <div style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0"}}><Spinner size={18}/><p style={{color:C.textMuted,fontSize:13}}>Wird generiert...</p></div>
            ) : d.recipe ? (
              <div>
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
                  <span style={{fontSize:24}}>{d.recipe.emoji||"🍽"}</span>
                  <div><p style={{fontWeight:600,fontSize:14}}>{d.recipe.name}</p><p style={{color:C.textMuted,fontSize:12}}>{d.recipe.time} - {d.recipe.difficulty}</p></div>
                </div>
...
export default function Mahlzeit() {
  ...
  const callAPI=async(finalPrefs,nope=null)=>{
    setScreen("loading");
    const restrictions=[...((activeProfile&&activeProfile.diet)||[]),...((activeProfile&&activeProfile.custom)||[])];
    const lovedRecipes=store.recipes.load(activeProfile ? activeProfile.id : null).filter(r=>r.status==="loved").slice(0,8).map(r=>r.name);
    try {
      ...
      setRecipe({name:"Pasta Aglio e Olio",emoji:"🍝",description:"Klassisch italienisch. Wenige Zutaten, maximaler Geschmack.",time:"20 Min",difficulty:"Einfach",calories:"ca. 420 kcal",ingredients:[{name:"Spaghetti",amount:"200g",available:true},{name:"Knoblauch",amount:"4 Zehen",available:true},{name:"Olivenoel",amount:"4 EL",available:false},{name:"Petersilie",amount:"1 Bund",available:false}],steps:["Pasta al dente kochen.","Knoblauch in Oel goldbraun anbraten.","Pasta abgiessen, Kochwasser aufheben.","Alles vermengen und servieren."],tip:"Das Kochwasser macht die Sauce cremig!"});
      setScreen("recipe");
    }
  };
